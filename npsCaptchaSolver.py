...
def npsPredictCaptchaDocker(imagepath_list, image_key='1', port_number='8502'):
...
def npsPredictCaptchaLite(imagepath_list): # Not currently working
    # Load TFLite model and allocate tensors.


    img = imagepath_list[0]
    interpreter = tf.lite.Interpreter(model_path="C:\\Users\\ntech\\OneDrive\\Documents\\Coding\\Captcha_Solver_App\\captchaSolverModel\\captcha_solver_tflite.tflite")
    ...
    #tf.io.readFile(), tf.io.decode_image()
    #img = cv2.imread(img)
    ...
    interpreter = tf.lite.Interpreter(model_path="captchaSolverModel\\captcha_solver_tflite.tflite")
    interpreter.allocate_tensors()
...
def main():
    ...
    while True: 
        ...
        prediction = npsPredictCaptchaDocker(imagepath_list)

        # This function uses a tflite version of the model
        #grayscale_path = folder + "resizedGrayscaleCaptchas\\"
        #prediction = npsPredictCaptchaLite(imagepath_list)
